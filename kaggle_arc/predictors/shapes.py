...
class MirrorPredictor(Predictor, mixins.AvailableMirror):
    ...
    def train(self, iodata_list):
        self.mirror = Mirror(
            self.m1, self.m2, vertical=self.vertical, horizontal=self.horizontal
        )
...
def change_colors(data, background_colors=[]):
    ...
    # redraw
    # print(colormap)
    data_modified = np.asarray([[colormap[c] for c in line] for line in data])
...
class ConstantShaper(Predictor):
    ...
    def is_available(self, iodata_list):
        colormaps = [
            change_colors(
                iodata.output_field.data,
                background_colors=[
                    c
                    for c in range(10)
                    if np.sum(iodata.input_field.data == c)
                    >= np.sum(iodata.output_field.data == c)
                ],
            )
        ...
        if (
            np.stack(colormaps).std(0).max() > 0
            or len(np.unique(np.stack(colormaps))) == 1
        ):
            for background_colors in [[]] + [[i] for i in range(10)]:
                colormaps = [
                    change_colors(
                        iodata.output_field.data, background_colors=background_colors
                    )
                    for iodata in iodata_list
                ]
    ...
    def predict(self, field):
        ...
        if self.input_pattern is not None:
            color_convertor = dict(
                set(
                    list(
                        zip(
                            *np.stack(
                                [self.input_pattern, self.crop_func(field.data)], 0
                            ).reshape(2, -1)
                        )
                    )
                )
            ...
            result = np.asarray(
                [[color_convertor.get(x, x) for x in line] for line in self.pattern]
            )
            result = Field(result)
        ...
        colormap = {
            i: [
                c
                for c in np.unique(data[np.where(ss == i)])
