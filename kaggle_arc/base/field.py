...
class Field:
    """ Represents the matrix with values [0-9], which corresponds to single input or output of ARC puzzles. Stores additional properties and colorization schemes, method for drawing, etc.
    """
    ...
    def __init__(self, data):
        if isinstance(data, list):
            self.data = np.asarray(
                [[(x if x >= 0 else 10 - x) for x in line] for line in data],
                dtype=np.uint8,
            )
        else:
            self.data = data.copy()
        ...
        self.colormap = None
        self.prop_names = (
            "h w xmin ymin xmax ymax xmean ymean is_convex holes contour_size interior_size".split()
            + "is_rectangular is_square".split()
    ...
    def show(self, ax=None, label=None):
        if ax is None:
            plt.figure(
                figsize=(self.width * self.multiplier, self.height * self.multiplier)
            )
            ax = plt.gca()
        ax.imshow(self.data, cmap=COLORMAP, norm=NORM)
        for edge, spine in ax.spines.items():
            spine.set_visible(False)
        ax.set_xticks(np.arange(self.data.shape[1]) + 0.5, minor=True)
        ax.set_yticks(np.arange(self.data.shape[0]) + 0.5, minor=True)
        ax.set_xticklabels([])
        ax.set_yticklabels([])
    ...
    @staticmethod
    def dice(a, b):
        dist = [multiclass_dice(a, b, i) for i in range(10)]
    ...
    def consts(self, value=1, multiplier=1):
        ...
        return Field(value * np.ones(new_shape, dtype=self.data.dtype))

    @staticmethod
    def fromstring(s):
        assert s[0] == "|"
        data = [[int(x) for x in line] for line in s[1:-1].split("|")]
        return Field(data)
    ...
    def build_nxgraph(self, connectivity={0: 4}, properties=None):
        ...
        graph_nx.graph["global_features"] = np.asarray(
            [[(np.sum(self.data == i) > 0) * 1.0 for i in range(10)]]
        ).astype(np.float64)
        ...
        for i in range(self.data.shape[0]):
            for j in range(self.data.shape[1]):
                ...
                neighbours = [
                    ...
                    and i1 >= 0
                    and j1 >= 0
                    and i1 < self.data.shape[0]
                    and j1 < self.data.shape[1]
                ]
                if connectivity.get(color, 4) == 4:
                    neighbours = [
                        (i1, j1) for i1, j1 in neighbours if (i1 == i or j1 == j)
                    ]
                # angle 90
                ...
                for d1, d2 in [(-1, -1), (-1, +1), (+1, -1), (+1, +1)]:
                    ...
                    if self.get(i + d1, j + d2) != color:
                        angle_270 = (
                            self.get(i, j + d2) == color
                            and self.get(i + d1, j) == color
                        )
                        left_shift = self.get(i + d1, j) != color
                        top_shift = self.get(i, j + d2) != color
                    angle_props.extend([angle_270, left_shift, top_shift])
                ...
                for i1, j1 in neighbours:
                    ...
                    if color0 == color1:
                        graph_nx.add_edge(
                            ...
                            id1,
                            features=np.asarray(
                                [(color0 == x) * 1.0 for x in range(10)]
                            ).astype(np.float64),
                        )
...
class ComplexField:
    ...
    def map(self, func):
        new_data = [[func(x) for x in line] for line in self.data]
