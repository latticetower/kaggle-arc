...
def walk_deps(
    ...
    for path in paths:
        ...
        for w in walk_deps(
            path, {*processed, *new_processed}, local_dirs=local_dirs, basedir=basedir
        ):
            yield w
...
def wrap2cell(data, ctype="code"):
    code_params = {"code": {"execution_count": 0, "outputs": []}}
    base = {"cell_type": ctype, "metadata": {}, "source": data}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "mainpy", help="path or name of main file which contains runnable code"
    )
