## TODO: this was unfinished and shouldn't be used now
import rootutils
...
def solve_task(iodata_list, max_steps=10, num_epochs=100, device="cpu"):
    ...
    for num_steps in range(1, max_steps):
        ...
        for e in range(num_epochs):
            ...
            for sample in task:
                ...
                x = (
                    torch.from_numpy(inp2img(sample["input"]))
                    .unsqueeze(0)
                    .float()
                    .to(device)
                )
                ...
                y_in = (
                    torch.from_numpy(inp2img(sample["output"]))
                    .unsqueeze(0)
                    .float()
                    .to(device)
                )
...
class CAMPredictor(Predictor, mixins.AvailableEqualShape):
    ...
    def train(self, iodata_list):
        ...
        for num_steps in range(1, self.max_steps):
            optimizer = torch.optim.Adam(
                self.model.parameters(), lr=(0.1 / (num_steps * 2))
            )
            ...
            for e in range(self.num_epochs):
                ...
                for iodata in iodata_list:
                    # predict output from input
                    x = (
                        torch.from_numpy(iodata.input_field.data_splitted)
                        .unsqueeze(0)
                        .float()
                        .to(self.device)
                    )
                    y = (
                        torch.from_numpy(iodata.output_field.data)
                        .long()
                        .unsqueeze(0)
                        .to(self.device)
                    )
                    ...
                    # enforces stability after solution is reached
                    y_in = (
                        torch.from_numpy(iodata.output_field.data_splitted)
                        .unsqueeze(0)
                        .float()
                        .to(self.device)
                    )
    ...
    def predict(self, field):
        ...
        with torch.no_grad():
            x = (
                torch.from_numpy(field.data_splitted)
                .unsqueeze(0)
                .float()
                .to(self.device)
            )
            ...
            for offset0, offset1 in np.argwhere(offsets == 1):
                ...
                for start, end in self.transitions:
                    ...
                    if offset0 + h > data.shape[0] or offset1 + w > data.shape[1]:
                        ...
                        continue
                    if np.all(
                        data[offset0 : offset0 + h, offset1 : offset1 + w] == start
                    ):
