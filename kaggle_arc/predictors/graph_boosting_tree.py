...
class GraphFeatureExtractor:
    ...
    def reorder(component_params_in, component_params_out):
        ...
        for i, comp in enumerate(component_params_out):
            for pos in comp["pos"]:
                comp_dict[pos] = i
        order = [comp_dict.get(list(comp["pos"])[0]) for comp in component_params_in]
    ...
    def get_data(cpi, cpo=None, use_zeros=True):
        if cpo is None:
            for gi in cpi:
                ...
                if "properties" in gi:
                    yield gi["color"], gi["features"], gi["comp_features"], gi[
                        "ncolors"
                    ], gi["size"], gi["properties"]
                else:
                    yield gi["color"], gi["features"], gi["comp_features"], gi[
                        "ncolors"
                    ], gi["size"]
        ...
        for gi, go in zip(cpi, cpo):
            ...
            yield gi["color"], gi["features"], gi["comp_features"], gi["ncolors"], gi[
                "size"
            ], target * 1.0, go["color"]
    ...
    def collect_graph_data(cpi, cpo=None, use_zeros=True):
        ...
        colors = np.asarray([[i == c for i in range(10)] for c in colors]).astype(
            np.float
        )
    ...
    def prepare_graph_features_diff(iodata, use_zeros=False):
        ...
        sizes = np.asarray(sizes).reshape(-1, 1)
        targets = np.asarray(
            [np.any(list(p)) * 1.0 for p in properties]
        )  # np.asarray(targets_bin)#.reshape(-1, 1)
    ...
    def prepare_graph_features_for_eval(field, use_zeros=True):
        ...
        inputs = GraphFeatureExtractor.collect_graph_data(
            graph_data, use_zeros=use_zeros
        )
...
class AvailableEqualShapeAndComponents:
    def is_available(self, iodata_list):
        ...
        for iodata in iodata_list:
            ...
            GO = iodata.output_field.build_nxgraph(
                connectivity={i: 4 for i in range(10)}
            )
            equal_shapes_of_components = GraphFeatureExtractor.compare_components(
                GI, GO
            )
...
class GraphBoostingTreePredictor2(Predictor):
    ...
    def train(self, iodata_list, n_estimators=20):
        ...
        for iodata in iodata_list:
            features, target_binary, target = (
                GraphFeatureExtractor.prepare_graph_features(iodata, self.use_zeros)
            )
...
class GraphBoostingTreePredictor3(Predictor, mixins.AvailableEqualShapeAndLessThanNComponents):
    ...
    def _make_train_binary_features(self, iodata_list):
        train_x_binary, train_y_binary = list(
            zip(
                *[
                    GraphFeatureExtractor.prepare_graph_features_diff(
                        iodata, self.use_zeros
                    )
                    for iodata in iodata_list
                ]
            )
